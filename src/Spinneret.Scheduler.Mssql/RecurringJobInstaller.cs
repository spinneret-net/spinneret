    ...
    ILogger<RecurringJobInstaller> logger) : IHostedService
{
    public async Task StartAsync(CancellationToken ct)
    {
        foreach (var job in jobs)
        {
            try
            {
                await scheduler.RegisterAsync(job.Key, job.CreateRequest(), job.Schedule, ct);
                logger.LogInformation("Installed recurring job '{Key}' ({Schedule}).", job.Key, job.Schedule);
            }
            catch (Exception ex)
            {
